...
init_connections()
{
  ...
  memset(users, 0, maxfd * sizeof(USER));		/* force start_time, buf to 0 */
...
prep_server()
{
  ...
  memcpy(&prev_info, sp, sizeof(SERVER));
...
void delete_server(int which)
{
  ...
  if ( server_count == 0 )
    ...
    {
      realloc( servers, (sizeof(SERVER) * server_count) );
      if ( servers == NULL )
	{
...
main_loop()
{
  ...
  while (1) {
    ...
    memcpy(&readfds, &readfd_set, sizeof(fd_set));
    memcpy(&writefds, &writefd_set, sizeof(fd_set));
